...
class DFSLResult:
    ...
    solution_path: Optional[Tuple[PathNode, ...]] = None
...
class IDAStar(GraphProblemSolver):
    ...
    def _binary_search_f_deepening(self, problem: GraphProblem) -> SearchResult:
        ...
        if zero_f_limit_result.solution_path is not None:
            return SearchResult(
                solver=self,
                problem=problem,
                final_search_node=zero_f_limit_result.solution_path,  # TODO: Fix it. set the whole path starting from the root.
        ...
        assert greedy_solution.final_search_node is not None
        greedy_solution_cost = greedy_solution.final_search_node.g_cost
        ...
        while calc_relative_error(low_f_limit, high_f_limit) > max_f_limit_relative_error:
            ...
            if result.solution_path is None:
        ...
        search_result = SearchResult(
            solver=self,
            problem=problem,
            final_search_node=result.solution_path,  # TODO: Fix it. set the whole path starting from the root.
    ...
    def _dfs_f(self, problem: GraphProblem, state: GraphProblemState, f_limit: float = 0,
               cumulative_cost: Optional[Cost] = None, last_operator_cost: Optional[Cost] = None, depth: int = 1) -> DFSLResult:
        ...
        if problem.is_goal(state):
            ...
            return DFSLResult(
                f_limit=f_limit, max_depth_reached=depth, solution_cost=cumulative_cost, solution_path=(PathNode(
        ...
        for successor_idx, operator_result in enumerate(problem.expand_state_with_costs(state), start=1):
            ...
            inner_call_result = self._dfs_f(
                problem, operator_result.successor_state,
                f_limit=f_limit,
                cumulative_cost=cumulative_cost + operator_result.operator_cost,
                last_operator_cost=operator_result.operator_cost, depth=depth + 1)
