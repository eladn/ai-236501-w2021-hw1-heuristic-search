...
class SearchNodesPriorityQueue:
    ...
    def clone(self) -> 'SearchNodesPriorityQueue':
        cloned = SearchNodesPriorityQueue()
        cloned._nodes_queue = heapdict(self._nodes_queue)
        cloned._state_to_search_node_mapping = dict(self._state_to_search_node_mapping)
        return cloned

    def to_log_format(self):
        cloned_heap = heapdict(self._nodes_queue)
        out_list = []
        while len(cloned_heap):
            node, prio = cloned_heap.popitem()
            out_list.append({'heap_prio': prio, 'node': node.to_log_format()})
        return out_list
...
class SearchNodesCollection:
    ...
    def clone(self) -> 'SearchNodesCollection':
        cloned = SearchNodesCollection()
        cloned._state_to_search_node_mapping = dict(self._state_to_search_node_mapping)
        return cloned

    def to_log_format(self):
        out_list = [node for _, node in self._state_to_search_node_mapping.items()]
        out_list.sort(key=lambda node: (node.expanding_priority, node.get_canonical_hash()))
        return [node.to_log_format() for node in out_list]
...
class BestFirstSearch(GraphProblemSolver):
    ...
    class IterationLogRecord:
        expand_number: int
        open_before_extract_next_node: SearchNodesPriorityQueue
        close_before_extract_next_node: Optional[SearchNodesPriorityQueue]
        node_to_expand: SearchNode
        open_after_extract_next_node: SearchNodesPriorityQueue
        close_after_extract_next_node: Optional[SearchNodesPriorityQueue]

        def to_log_format(self):
            return {
                'expand_number': self.expand_number,
                'open_before_extract_next_node': self.open_before_extract_next_node.to_log_format(),
                'close_before_extract_next_node': self.close_before_extract_next_node.to_log_format(),
                'node_to_expand': self.node_to_expand.to_log_format(),
                'open_after_extract_next_node': self.open_after_extract_next_node.to_log_format(),
                'close_after_extract_next_node': self.close_after_extract_next_node.to_log_format()
            }

    class ExecutionLog:
        iterations_log_records: List['BestFirstSearch.IterationLogRecord'] = list()

        def log_new_iteration_started(self, solver: 'BestFirstSearch'):
            cur_iteration_log_record = BestFirstSearch.IterationLogRecord()
            cur_iteration_log_record.open_before_extract_next_node = solver.open.clone()
            cur_iteration_log_record.close_before_extract_next_node = solver.close.clone()
            self.iterations_log_records.append(cur_iteration_log_record)
            cur_iteration_log_record.expand_number = len(self.iterations_log_records)

        def log_next_node_to_expand_extracted(self, solver: 'BestFirstSearch', node_to_expand: SearchNode):
            assert len(self.iterations_log_records) > 0
            cur_iteration_log_record = self.iterations_log_records[-1]
            cur_iteration_log_record.node_to_expand = node_to_expand
            cur_iteration_log_record.open_after_extract_next_node = solver.open.clone()
            cur_iteration_log_record.close_after_extract_next_node = solver.close.clone()

        def to_log_format(self):
            return [item.to_log_format() for item in self.iterations_log_records]

    solver_name: str = 'BestFirstSearch'

    def __init__(self, use_close: bool = True, store_execution_log: bool = False):
        self.open: SearchNodesPriorityQueue = None
        self.close: Optional[SearchNodesCollection] = None
        self.use_close = use_close
        self.store_execution_log = store_execution_log
        self.last_execution_log: Optional[BestFirstSearch.ExecutionLog] = None
    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        ...
        self.last_execution_log = BestFirstSearch.ExecutionLog() if self.store_execution_log else None
        ...
        with Timer(print_title=False) as timer:
            ...
            while True:
                if self.store_execution_log:
                    self.last_execution_log.log_new_iteration_started(self)
                ...
                if self.store_execution_log:
                    self.last_execution_log.log_next_node_to_expand_extracted(self, next_node_to_expand)

                # TODO [staff]: is it correct to increment here? maybe should be after the `is_goal` check?
